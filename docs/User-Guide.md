...
从 [GitHub Releases](https://github.com/wismyzhizi2018/Abu-Cowork/releases) 下载对应平台的安装包：
...
如果 Abu 启动后显示登录页面，说明当前版本已启用 ERP 登录功能：

1. 输入公司 ERP 手机号和密码
2. 点击 **「登录」**
3. 登录成功后，模型配置会自动同步，无需手动填写 API Key

> 也可以点击 **「跳过，直接使用」** 跳过登录，但需要手动配置 AI 服务。

### 3. 配置模型

如果未启用登录功能（或跳过了登录），需要手动配置：
...
> 如果通过 ERP 登录，模型配置会自动同步到 AI 服务列表中（标记为「远程」来源），无需手动配置。你仍然可以手动添加其他服务商。
